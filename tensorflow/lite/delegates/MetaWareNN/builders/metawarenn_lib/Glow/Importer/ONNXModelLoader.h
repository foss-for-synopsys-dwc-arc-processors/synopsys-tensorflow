...
#include "../Importer/CommonOperatorLoader.h"

#include "../../onnx/onnx-ml.pb.h"// /onnx_pb.h"

#include "../LLVM/ADT/ArrayRef.h"
#include "../LLVM/ADT/StringRef.h"

#include <google/protobuf/io/coded_stream.h>//"google/protobuf/io/coded_stream.h"
#include <google/protobuf/io/zero_copy_stream_impl.h>//"google/protobuf/io/zero_copy_stream_impl.h"
#include <fstream>
#include <string>
...
Error loadTensor(const onnx::TensorProto &in, Tensor *T,
...
onnx::GraphProto parseOnnxFile(const std::string &fileName);
...
void fillPlaceholders(const onnx::GraphProto &parsedFile,
                      PlaceholderBindings *bindings,
                      std::vector<Tensor> *partialTensorPayloads = nullptr,
                      bool usingGlowCustomOps = false);
...
class ONNXModelLoader
    : public CommonOperatorLoader<onnx::NodeProto,
                                  onnx::AttributeProto> {
  ...
  Expected<ElemKind>
  convertTensorProtoDataType(onnx::TensorProto_DataType t);
  ...
  Error loadOperator(const onnx::NodeProto &op);
  ...
  Expected<bool> foldOperator(const onnx::NodeProto &op);

  /// Helper function to print better log information for operator failure cases
  const std::string opErrMsg(const onnx::NodeProto &op,
  ...
  Error loadNonZero(const onnx::NodeProto &op,
                    const ArgumentDictionaryTy &dict);

  /// Load Trigonometric Ops
  Error loadAsin(const onnx::NodeProto &op,
                 const ArgumentDictionaryTy &dict);

  Error loadAcos(const onnx::NodeProto &op,
                 const ArgumentDictionaryTy &dict);

  /// Load Erf ONNX operator
  Error loadErf(const onnx::NodeProto &op,
                const ArgumentDictionaryTy &dict);

  Error loadAtan(const onnx::NodeProto &op,
                 const ArgumentDictionaryTy &dict);

  /// Load Constant ONNX operator.
  Error loadConstant(const onnx::NodeProto &op,
                     ArgumentDictionaryTy &dict);

  /// Helper function for ONNX range operator
  template <typename T>
  Error getRange(const onnx::NodeProto &op, Constant *constT);

  /// Load Range ONNX operator
  Error loadRange(const onnx::NodeProto &op,
                  ArgumentDictionaryTy &dict);

  /// Load PRelu ONNX operator.
  Error loadPRelu(const onnx::NodeProto &op,
                  ArgumentDictionaryTy &dict);

  /// Load Slice ONNX operator.
  Error loadSlice(const onnx::NodeProto &op,
                  ArgumentDictionaryTy &dict);
  ...
  Error loadSign(const onnx::NodeProto &op,
                 const ArgumentDictionaryTy &dict);

  /// Load Softmax ONNX operator
  Error loadSoftmax(const onnx::NodeProto &op,
                    const ArgumentDictionaryTy &dict);

  /// Load Conv ONNX operator.
  Error loadConv(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load ChannelwiseQuantizedConvolution Glow operator.
  Error loadChannelwiseQuantizedConvolution(const onnx::NodeProto &op,
                                            ArgumentDictionaryTy &dict);
  ...
  Error loadTensorwiseQuantizedConvolution(const onnx::NodeProto &op,
                                           ArgumentDictionaryTy &dict);
  /// Load ConvTranspose ONNX operator.
  Error loadConvTranspose(const onnx::NodeProto &op,
                          ArgumentDictionaryTy &dict);
  ...
  Error loadConv1D(const onnx::NodeProto &op,
                   ArgumentDictionaryTy &dict);
  ...
  Error loadPool(const onnx::NodeProto &op,
  ...
  /// inputs.
  Error loadTensorwiseQuantizedPool(const onnx::NodeProto &op,
                                    ArgumentDictionaryTy &dict,
                                    llvm::StringRef typeName);

  /// Load GlobalAveragePool ONNX operator.
  Error loadGlobalAveragePool(const onnx::NodeProto &op,
                              ArgumentDictionaryTy &dict);

  /// Load Squeeze ONNX operator.
  Error loadSqueeze(const onnx::NodeProto &op,
                    ArgumentDictionaryTy &dict);

  /// Load Unsqueeze ONNX operator.
  Error loadUnsqueeze(const onnx::NodeProto &op,
                      ArgumentDictionaryTy &dict);

  /// Load ArgMax and ArgMin ONNX operators.
  Error loadArgMinMax(const onnx::NodeProto &op,
                      ArgumentDictionaryTy &dict, bool isMin);

  /// Load Upsample ONNX operator.
  Error loadUpsample(const onnx::NodeProto &op,
                     ArgumentDictionaryTy &dict);

  /// Load Resize ONNX Operator.
  Error loadResize(const onnx::NodeProto &op,
                   const ArgumentDictionaryTy &dict);

  /// Load BatchNormalization ONNX operator.
  Error loadBatchNormalization(const onnx::NodeProto &op,
                               ArgumentDictionaryTy &dict);

  /// Load Concat ONNX operator.
  Error loadConcat(const onnx::NodeProto &op,
                   ArgumentDictionaryTy &dict);

  /// Load FCTransposed ONNX operator.
  Error loadFCTransposed(const onnx::NodeProto &op,
                         ArgumentDictionaryTy &dict);

  /// Load Gemm ONNX operator.
  Error loadGemm(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load MatMul ONNX operator.
  Error loadMatMul(const onnx::NodeProto &op,
                   ArgumentDictionaryTy &dict);

  /// Load Pad ONNX operator.
  Error loadPad(const onnx::NodeProto &op,
                ArgumentDictionaryTy &dict);

  /// Load Cast ONNX operator.
  Error loadCast(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load LeakyRelu ONNX operator.
  Error loadLeakyRelu(const onnx::NodeProto &op,
                      ArgumentDictionaryTy &dict);

  /// Load SpaceToDepth ONNX operator.
  Error loadSpaceToDepth(const onnx::NodeProto &op,
                         ArgumentDictionaryTy &dict);

  /// Load ReduceL2 ONNX operator
  Error loadReduceL2(const onnx::NodeProto &op,
                     const ArgumentDictionaryTy &dict);

  /// Load DepthToSpace ONNX operator.
  Error loadDepthToSpace(const onnx::NodeProto &op,
                         const ArgumentDictionaryTy &dict);

  /// Load ConstantOfShape ONNX operator.
  Error loadConstantOfShape(const onnx::NodeProto &op,
                            ArgumentDictionaryTy &dict, bool isSplat);

  /// Load Tile ONNX operator.
  Error loadTile(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load Expand ONNX operator.
  Error loadExpand(const onnx::NodeProto &op,
                   const ArgumentDictionaryTy &dict);

  /// Load Where ONNX operator.
  Error loadWhere(const onnx::NodeProto &op,
                  ArgumentDictionaryTy &dict);

  /// Load RNN ONNX operator.
  Error loadRNN(const onnx::NodeProto &op,
                ArgumentDictionaryTy &dict);

  /// Load GRU ONNX operator.
  Error loadGRU(const onnx::NodeProto &op,
                ArgumentDictionaryTy &dict);

  /// Load LSTM ONNX operator.
  Error loadLSTM(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load Clip ONNX operator.
  Error loadClip(const onnx::NodeProto &op,
                 const ArgumentDictionaryTy &dict);

  /// Load Glow specific operators, not defined in ONNX format
  /// Load Glow CmpEQ operator.
  Error loadCmpEQ(const onnx::NodeProto &op,
                  ArgumentDictionaryTy &dict);

  /// Load Glow CmpLTE operator.
  Error loadCmpLTE(const onnx::NodeProto &op,
                   ArgumentDictionaryTy &dict);

  /// Load Mean ONNX operator
  Error loadMean(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load Glow Select operator.
  Error loadSelect(const onnx::NodeProto &op,
                   ArgumentDictionaryTy &dict);

  /// Load Glow Quantize operator.
  Error loadQuantize(const onnx::NodeProto &op,
                     ArgumentDictionaryTy &dict);

  /// Load Onnx QuantizeLinear operator.
  Error loadQuantizeLinear(const onnx::NodeProto &op,
                           ArgumentDictionaryTy &dict);

  /// Load Glow ConvertTo operator.
  Error loadConvertTo(const onnx::NodeProto &op,
                      ArgumentDictionaryTy &dict);

  /// Load Glow Dequantize operator.
  Error loadDequantize(const onnx::NodeProto &op,
                       ArgumentDictionaryTy &dict);

  /// Load Glow Regression operator.
  Error loadRegression(const onnx::NodeProto &op,
                       ArgumentDictionaryTy &dict);

  /// Load Glow BatchedAdd operator.
  Error loadBatchedAdd(const onnx::NodeProto &op,
                       ArgumentDictionaryTy &dict);

  /// Load Glow CumSum operator.
  Error loadCumSum(const onnx::NodeProto &op,
                   ArgumentDictionaryTy &dict);

  /// Load Glow ScatterAssign operator.
  Error loadScatterAssign(const onnx::NodeProto &op,
                          ArgumentDictionaryTy &dict);

  /// Load Glow IntLookupTable operator.
  Error loadIntLookupTable(const onnx::NodeProto &op,
                           ArgumentDictionaryTy &dict);

  /// Load Glow LengthsRangeFill operator.
  Error loadLengthsRangeFill(const onnx::NodeProto &op,
                             ArgumentDictionaryTy &dict);

  /// Load Glow RescaleQuantized operator.
  Error loadRescaleQuantized(const onnx::NodeProto &op,
                             ArgumentDictionaryTy &dict);

  /// Load Glow RowwiseQuantizedSparseLengthsWeightedSum operator.
  Error loadRowwiseQuantizedSparseLengthsWeightedSum(
      const onnx::NodeProto &op, ArgumentDictionaryTy &dict);
  ...
  Error
  loadFusedRowwiseQuantizedSparseLengthsSum(const onnx::NodeProto &op,
                                            ArgumentDictionaryTy &dict);

  /// Load Glow RowwiseQuantizedFullyConnected operator.
  Error loadRowwiseQuantizedFullyConnected(const onnx::NodeProto &op,
                                           ArgumentDictionaryTy &dict);

  /// Load Glow FullyConnected operator.
  Error loadFullyConnected(const onnx::NodeProto &op,
                           ArgumentDictionaryTy &dict);

  /// Load ONNX Identity operator.
  Error loadIdentity(const onnx::NodeProto &op,
                     ArgumentDictionaryTy &dict);

  /// Load Glow Splat operator.
  Error loadSplat(const onnx::NodeProto &op,
                  ArgumentDictionaryTy &dict);

  /// Load NonMaxSuppression ONNX and TF NMSv4 operator.
  /// The \p isV4 indicates whether this is ONNX or custom NMSv4 operator.
  Error loadNonMaxSuppression(const onnx::NodeProto &op,
                              ArgumentDictionaryTy &dict, bool isV4);

  /// Load Glow InsertTensor operator.
  Error loadInsertTensor(const onnx::NodeProto &op,
                         ArgumentDictionaryTy &dict);

  /// Load If ONNX operator.
  Error loadIf(const onnx::NodeProto &op,
               const ArgumentDictionaryTy &dict);
  ...
  Error loadAdaptiveAvgPool(const onnx::NodeProto &op,
                            ArgumentDictionaryTy &dict);

  /// Load Flip Glow operator.
  Error loadFlip(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  /// Load AudioSpectrogram Glow operator.
  Error loadAudioSpectrogram(const onnx::NodeProto &op,
                             ArgumentDictionaryTy &dict);

  /// Load Loop operator.
  Error loadLoop(const onnx::NodeProto &op,
                 const ArgumentDictionaryTy &dict);

  /// Load MFCC Glow operator.
  Error loadMFCC(const onnx::NodeProto &op,
                 ArgumentDictionaryTy &dict);

  // Load ROIAlign ONNX operator
  Error loadROIAlign(const onnx::NodeProto &op,
                     ArgumentDictionaryTy &dict);
...
protected:
  ...
  Error loadNetwork(onnx::GraphProto &net,
  ...
  Expected<Constant *>
  replaySerializedConstFold(const onnx::TensorProto &in,
                            onnx::GraphProto &net);
  ...
  Error loadInputs(onnx::GraphProto &net,
  ...
  static Expected<onnx::ModelProto> loadProto(const void *onnxModel,
  ...
  /// \p tensorNames.
  Error checkInputs(onnx::GraphProto &net,
  ...
  Error collectStaticInputs(onnx::GraphProto &net);
  ...
  Error setupOrigStaticTypeMap(onnx::GraphProto &net);
  ...
  Error assignGraphInputs(const onnx::GraphProto &net,
  ...
  Expected<Type> getTensorType(const onnx::ValueInfoProto &in);

  /// \returns a Type with the proper shape and element type given \p in.
  Expected<Type> getTensorType(const onnx::TensorProto &in);

  /// Load a model \p modelDef given \p tensorNames, \p types, \p B, and
  /// \p loadInputsAsPlaceholdersForOnnx.
  Error loadModel(onnx::ModelProto &modelDef,
                  llvm::ArrayRef<const char *> tensorNames,
  ...
  /// \p numPartitions.
  Error setupPartitions(onnx::ModelProto &modelDef,
  ...
  void setupPositionalIO(const onnx::GraphProto &graph);
  ...
  Error setupUpdatedTQPMap(onnx::ModelProto &modelDef,
